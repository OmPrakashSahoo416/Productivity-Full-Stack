import { CreateCardCopy } from "@/actions/create-card";
...
export default function CardEdit({cardId, list, children}:{cardId:string, list : {
  ...
  return (
    ...
    <Button onClick={() => CreateCardCopy({newtitle:card?.title as string, newdesc:card?.description as string, newlistId:card?.listId as string, newboardId:list.boardId as string})} variant={"ghost"} className="rounded-md border border-rose-200  hover:text-slate-100 hover:bg-slate-500 p-1 mr-1 text-xs  h-auto w-auto" ><Copy size={12}></Copy></Button>

    <Button onClick={() => DeleteCard(card?.id as string, list?.boardId as string)}    variant={"ghost"} className="rounded-md border border-rose-200  hover:text-slate-100 hover:bg-rose-500 p-1 mr-1 text-xs  h-auto w-auto" ><X size={12}></X></Button>
