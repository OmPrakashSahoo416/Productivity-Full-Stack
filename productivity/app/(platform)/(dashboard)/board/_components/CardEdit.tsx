...
export default function CardEdit({cardId, list, children}:{cardId:string, list : {
  ...
  return (
    ...
    <Button onClick={() => DeleteCard(card?.id as string, list?.boardId as string)}  variant={"ghost"} className="rounded-md  hover:text-slate-100 hover:bg-rose-500 p-1 mr-1 h-auto w-auto" ><X size={15}></X></Button>
    </div>
